...
class Quist extends CI_Controller {
	...
	public function __construct() {
		...
		$this->load->model('Quist_model');
	}

	public function index($page=1) {
        $page = floor(($page/10) + 1);
        $getData				= $this->Quist_model->getData($page);
		...
		$data['head'] 			= 'Evaluasi';
		$data['content']		= 'Soal-soal evaluasi';
		$data['title']			= 'Evaluasi';
		$data['node_modules']	= 'sweetalert2/dist/sweetalert2.all.min.js';
		$data['script']			= 'quist/quist-list.js';
		$this->load->view('templates/header', $data);
		$this->load->view('pages/quist/quist-list');
		$this->load->view('templates/footer');
	}

	public function createQuist() {
		$data['head'] 			= 'Evaluasi';
		$data['content']		= 'Soal-soal evaluasi';
		$data['title']			= 'Tambah Evaluasi';
		$data['script']			= 'quist/quist-create.js';
		$this->load->view('templates/header', $data);
		$this->load->view('pages/quist/quist-create');
		$this->load->view('templates/footer');
	}

	public function postQuist() {
		$results	= [
			'status'	=> TRUE,
			'message'	=> 'Pertanyaan evaluasi berhasil di tambahkan.'
		];

		$this->form_validation->set_rules('question', 'Pertanyaan', 'trim|required');
		$this->form_validation->set_rules('answer_a', 'Jawaban A', 'trim|required');
		$this->form_validation->set_rules('answer_b', 'Jawaban B', 'trim|required');
		$this->form_validation->set_rules('answer_c', 'Jawaban C', 'trim|required');
		$this->form_validation->set_rules('answer_d', 'Jawaban D', 'trim|required');
		if($this->form_validation->run()) {
			$postData = $this->Quist_model->postData();
			if(!$postData->status) {
				$results	= [
					'status'	=> FALSE,
					'message'	=> 'Pertanyaan evaluasi gagal di tambahkan.'
				];
			}
		}else {
			$results	= [
		...
		redirect(base_url('quist/createQuist'));
	}

    public function history($page=1) {
        $page = floor(($page/10) + 1);
        $getData				= $this->Quist_model->getDataHistory($page);
    ...
    public function detailhistory($_id) {
		$getData				= $this->Quist_model->getDetailDataHistory($_id);
		$data['data']			= $getData->data;
		$data['head'] 			= 'Riwayat Evaluasi';
		$data['content']		= 'Riwayat Evaluasi';
	...
	public function updateKompetensi($_id) {
		$getData				= $this->Quist_model->getDetailData(KOMPETENSI, $_id);
		$data['_id']			= $_id;
		$data['data']			= $getData->data;
	...
	public function materi() {
		$getData				= $this->Quist_model->getData(MATERI);
		$data['data']			= $getData->data;
		$data['head'] 			= 'Konten Aplikasi';
	...
	public function postMateri() {
		...
		if($this->form_validation->run()) {
			$postData = $this->Quist_model->postData(MATERI);
			if(!$postData->status) {
				$results	= [
					'status'	=> FALSE,
	...
	public function updateMateri($_id) {
		$getData				= $this->Quist_model->getDetailData(MATERI, $_id);
		$data['_id']			= $_id;
		$data['data']			= $getData->data;
		$data['head'] 			= 'Konten Aplikasi';
		$data['content']		= 'Konten materi';
	...
	public function tentang() {
		$getData				= $this->Quist_model->getData(TENTANG);
		$data['data']			= $getData->data;
		$data['head'] 			= 'Konten Aplikasi';
	...
	public function postTentang() {
		...
		$this->form_validation->set_rules('description', 'Isi', 'trim|required');
		if($this->form_validation->run()) {
			$postData = $this->Quist_model->postData(TENTANG);
			if(!$postData->status) {
				$results	= [
					'status'	=> FALSE,
	...
	public function updateTentang($_id) {
		$getData				= $this->Quist_model->getDetailData(TENTANG, $_id);
		$data['_id']			= $_id;
		$data['data']			= $getData->data;
		$data['head'] 			= 'Konten Aplikasi';
		$data['content']		= 'Konten tentang';
	...
	public function bantuan() {
		$getData				= $this->Quist_model->getData(BANTUAN);
		$data['data']			= $getData->data;
		$data['head'] 			= 'Konten Aplikasi';
	...
	public function postBantuan() {
		...
		$this->form_validation->set_rules('description', 'Isi', 'trim|required');
		if($this->form_validation->run()) {
			$postData = $this->Quist_model->postData(BANTUAN);
			if(!$postData->status) {
				$results	= [
					'status'	=> FALSE,
	...
	public function updateBantuan($_id) {
		$getData				= $this->Quist_model->getDetailData(BANTUAN, $_id);
		$data['_id']			= $_id;
		$data['data']			= $getData->data;
		$data['head'] 			= 'Konten Aplikasi';
		$data['content']		= 'Konten bantuan';
	...
	public function putData($_id) {
		...
		$this->form_validation->set_rules('description', 'Isi', 'trim|required');
		if($this->form_validation->run()) {
			$putData = $this->Quist_model->putData($_id);
			if(!$putData->status) {
				$results	= [
					'status'	=> FALSE,
