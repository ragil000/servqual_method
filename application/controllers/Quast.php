...
class Quast extends CI_Controller {
	...
	public function __construct() {
		...
		$this->load->model('Quast_model');
	}

	public function index($page=1) {
        $page = floor(($page/10) + 1);
        $getData				= $this->Quast_model->getData($page);

        //konfigurasi pagination
        $config['base_url'] = site_url('quast'); //site url
		...
		$data['script']			= 'quast/quast-list.js';
		$this->load->view('templates/header', $data);
		$this->load->view('pages/quast/quast-list');
	...
	public function postQuast() {
		...
		if($this->form_validation->run()) {
			$postData = $this->Quast_model->postData();
			if(!$postData->status) {
				$results	= [
					'status'	=> FALSE,
    ...
    public function history($page=1) {
        $page = floor(($page/10) + 1);
        $getData				= $this->Quast_model->getDataHistory($page);
    ...
    public function detailhistory($_id) {
		$getData				= $this->Quast_model->getDetailDataHistory($_id);
		$data['data']			= $getData->data;
		$data['head'] 			= 'Riwayat Evaluasi';
		$data['content']		= 'Riwayat Evaluasi';
	...
	public function updateKompetensi($_id) {
		$getData				= $this->Quast_model->getDetailData(KOMPETENSI, $_id);
		$data['_id']			= $_id;
		$data['data']			= $getData->data;
	...
	public function materi() {
		$getData				= $this->Quast_model->getData(MATERI);
		$data['data']			= $getData->data;
		$data['head'] 			= 'Konten Aplikasi';
	...
	public function postMateri() {
		...
		if($this->form_validation->run()) {
			$postData = $this->Quast_model->postData(MATERI);
			if(!$postData->status) {
				$results	= [
					'status'	=> FALSE,
	...
	public function updateMateri($_id) {
		$getData				= $this->Quast_model->getDetailData(MATERI, $_id);
		$data['_id']			= $_id;
		$data['data']			= $getData->data;
		$data['head'] 			= 'Konten Aplikasi';
		$data['content']		= 'Konten materi';
	...
	public function tentang() {
		$getData				= $this->Quast_model->getData(TENTANG);
		$data['data']			= $getData->data;
		$data['head'] 			= 'Konten Aplikasi';
	...
	public function postTentang() {
		...
		$this->form_validation->set_rules('description', 'Isi', 'trim|required');
		if($this->form_validation->run()) {
			$postData = $this->Quast_model->postData(TENTANG);
			if(!$postData->status) {
				$results	= [
					'status'	=> FALSE,
	...
	public function updateTentang($_id) {
		$getData				= $this->Quast_model->getDetailData(TENTANG, $_id);
		$data['_id']			= $_id;
		$data['data']			= $getData->data;
		$data['head'] 			= 'Konten Aplikasi';
		$data['content']		= 'Konten tentang';
	...
	public function bantuan() {
		$getData				= $this->Quast_model->getData(BANTUAN);
		$data['data']			= $getData->data;
		$data['head'] 			= 'Konten Aplikasi';
	...
	public function postBantuan() {
		...
		$this->form_validation->set_rules('description', 'Isi', 'trim|required');
		if($this->form_validation->run()) {
			$postData = $this->Quast_model->postData(BANTUAN);
			if(!$postData->status) {
				$results	= [
					'status'	=> FALSE,
	...
	public function updateBantuan($_id) {
		$getData				= $this->Quast_model->getDetailData(BANTUAN, $_id);
		$data['_id']			= $_id;
		$data['data']			= $getData->data;
		$data['head'] 			= 'Konten Aplikasi';
		$data['content']		= 'Konten bantuan';
	...
	public function putData($_id) {
		...
		$this->form_validation->set_rules('description', 'Isi', 'trim|required');
		if($this->form_validation->run()) {
			$putData = $this->Quast_model->putData($_id);
			if(!$putData->status) {
				$results	= [
					'status'	=> FALSE,
